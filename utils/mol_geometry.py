...
def ic_to_xyz(p1, p2, p3, d34, a234, t1234):
    '''
    Computes the cartesian coordinates of atom 4 given the cartesian coordinates of the other three atoms and its corresponding internal coordinates. 
    - Arguments:
    ...
    D_prima[:, 0, 0] = d34 * torch.cos(a234)
    D_prima[:, 1, 0] = d34 * torch.sin(a234) * torch.cos(t1234)  
    D_prima[:, 2, 0] = d34 * torch.sin(a234) * torch.sin(t1234)
    ...
    n = torch.cross(x12, x23, dim = 1)
    n /= torch.norm(n.clone(), dim = 1, keepdim = True)
